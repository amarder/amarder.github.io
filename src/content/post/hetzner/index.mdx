...
Your Hetzner server also has an IPv6 address. If you set up an AAAA record with that address, then users will be able to connect to the server faster (I recently learned about DNS dual-stack). You can find your server's IPv6 address in the [Hetzner Console](https://console.hetzner.com/):

1. Click **Add Record** again
2. Create a wildcard AAAA record:
   - **Type**: `AAAA`
   - **Name**: `*.k3s`
...
:::tip{title="Updating a Cluster"}
You can re-run `hetzner-k3s create` with an updated `cluster.yaml` to update an existing cluster. Many, but not all, configuration changes are supported after initial creation.
:::
