...
hetzner-k3s deploys a cluster autoscaler, but it's not needed for a single-node cluster with no worker pools. It will sit in `CrashLoopBackOff` indefinitely, so remove it:
