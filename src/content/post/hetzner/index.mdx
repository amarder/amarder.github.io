...
1. I've been self-hosting an email newsletter from home. With power outages, internet outages, home server outages, the server downtime was going to be a bit much for my taste. I wanted to deploy to the cloud for better uptime.

2. I'm cheap. I had spun up a two node Kubernetes cluster on DigitalOcean and that was costing about $20 a month. It didn't feel like I was getting much for my money.
...
- Better uptime (compared to my homeserver) ✔️
- Cheap ✔️
...
- [hetzner-k3s](https://github.com/vitobotta/hetzner-k3s?tab=readme-ov-file#quick-start) installed
- [kubectl](https://kubernetes.io/docs/tasks/tools/), [helm](https://helm.sh/docs/intro/install/), [helmfile](https://helmfile.readthedocs.io/en/latest/#installation), and the [helm-secrets](https://github.com/jkroepke/helm-secrets/wiki/Installation) and [helm-diff](https://github.com/databus23/helm-diff?tab=readme-ov-file#install) plugins installed
- [sops](https://github.com/getsops/sops/releases) and [age](https://github.com/FiloSottile/age?tab=readme-ov-file#installation) installed (for encrypting secrets)
...
- **IPv4 included.** I was tempted save $0.60/month by going IPv6-only (`public_network.ipv4: false`), but IPv4 means you can SSH in from any network, all visitors can reach your services regardless of their ISP, and you don't have to troubleshoot IPv6 connectivity issues. Overall, I think it's money well-spent.
...
For certificate issuance, I chose [DNS-01 challenges](https://cert-manager.io/docs/configuration/acme/dns01/) (supported by [cert-manager](https://cert-manager.io/)) via Cloudflare. This lets us get a single wildcard certificate for all services and avoids some common routing issues associated with HTTP-01.

1. Go to [Cloudflare API Tokens](https://dash.cloudflare.com/profile/api-tokens)
2. Click **Create Token** → use the **Edit zone DNS** template
3. Under **Zone Resources**, select your domain
4. (Optional) I chose to set up Client IP Address Filtering, setting the Operator to "Is in" and adding my server's public IP address to the Value list. This way the API token should only work from that one IP address.
...
Open `https://keila.k3s.andrewmarder.net` in your browser and log in with the credentials from your secret. SMTP is configured at deploy time via `secrets/keila.yaml`, so you can create a sender under **Senders** → **Create** right away.
...
I like hetzner-k3s, but... I wish it could modify existing clusters. There were a couple times when my cluster wasn't doing what I wanted. I edited my `cluster.yaml` file, but there was no way to have hetzner-k3s update my existing cluster. If I knew how to assign existing and available Primary IPs in Hetzner that could be another way to solve this problem.
:::
