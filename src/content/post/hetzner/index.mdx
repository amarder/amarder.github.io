...
Cloudflare's free plan doesn't proxy wildcard records. The grey cloud (DNS only) means traffic goes directly to your server. TLS termination is handled by Traefik on the server itself, with certificates managed by cert-manager.
:::
