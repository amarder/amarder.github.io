...
I just ran an experiment where I created a Google Calendar event inviting my Proton Mail and Fastmail accounts. I RSVP'ed yes from both email accounts. When I open my Google Calendar I can see the response from my Proton Mail account but not my Fastmail account. Why is this happening?
...
2. **iTIP** (RFC 5546) - the iCalendar Transport-Independent Interoperability Protocol, which defines the semantics of calendar scheduling messages: `REQUEST` (invitation), `REPLY` (RSVP), `CANCEL`, etc.
...
sequenceDiagram
    ...
    GC->>GM: User creates event with attendee
    GM->>FE: REQUEST invite via SMTP
    FE->>FC: Parse iCalendar, add event
    FC->>FE: User RSVPs Yes
    FE->>GM: REPLY accepted via SMTP
